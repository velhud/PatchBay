...
Keep `codex-mcp-wrapper` Python/FastAPI first.

Port CodexPro ideas and selected code into the wrapper's Python architecture instead of permanently embedding a TypeScript sidecar.
...
- reimplement wrapper job engine, worktree lifecycle, and Codex CLI parsing;
...
The user goal is the strongest final app in the wrapper repo, not preservation of CodexPro's architecture.
...
- logs go through wrapper policy;
...
2. Implement Python services behind wrapper interfaces.
...
- real ChatGPT Developer Mode and public tunnel release evals;
