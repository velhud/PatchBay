...
PatchBay distinguishes Codex completing a turn from its CLI wrapper exiting. If the exact Codex session records `task_complete` while the wrapper remains alive, PatchBay preserves the final report and cleans up the wrapper automatically. This cleanup happens only after authoritative completion and is not a timeout on long-running work.
