...
| `codex_worker_status` | read-only | Return the compact pull-based team status bar: status counts, deltas since last check, suggested action, and one short line per worker. |
...
5. Use `codex_worker_status` while workers run; inspect worker reports, liveness/checkpoints, and exact changes when needed.
