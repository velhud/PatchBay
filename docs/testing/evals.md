...
- PatchBay `PYTHONDONTWRITEBYTECODE=1 python -m pytest tests -q`: passed for the integrated Hub/Edge reconciliation repair candidate with `972 passed, 4 skipped` on macOS. The 976-test inventory includes marker-stripped detached-descendant cleanup, restart cleanup from persisted exact descendant identity, atomic Edge projection rollback/retry, bounded fleet-status output under large worker history, supervisor cleanup under host contention, and group-scoped retirement of pre-atomic manifestless batch parents after every observed child is terminal.
