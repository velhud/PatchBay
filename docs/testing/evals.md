...
- PatchBay `PYTHONDONTWRITEBYTECODE=1 python -m pytest tests -q`: passed for the integrated Hub/Edge reconciliation repair candidate with `973 passed, 4 skipped` on macOS. The 977-test inventory includes marker-stripped detached-descendant cleanup, restart cleanup from persisted exact descendant identity, atomic Edge projection rollback/retry, bounded fleet and group status under 20,000-operation history, supervisor cleanup under host contention, and group-scoped retirement of pre-atomic manifestless batch parents after every observed child is terminal.
