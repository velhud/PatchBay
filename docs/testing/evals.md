...
- PatchBay `PYTHONDONTWRITEBYTECODE=1 python -m pytest tests -q`: passed for the integrated Hub/Edge reconciliation repair candidate with `967 passed, 4 skipped` on macOS and `969 passed, 2 skipped` in the production Linux environment. Both runs cover the same 971-test inventory with platform-specific skips. The final Linux pass includes marker-stripped detached-descendant cleanup, restart cleanup from persisted exact descendant identity, atomic Edge projection rollback/retry, bounded fleet-status output under large worker history, and supervisor cleanup under host contention.
