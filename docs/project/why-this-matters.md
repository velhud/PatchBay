...
`patchbay` exists to remove that manual bridge. It exposes local workspace context and Codex CLI workflows through a local Streamable HTTP MCP server so ChatGPT or another MCP-compatible client can inspect allowed repos, delegate work to local Codex, and review results without importing the whole repository into a chat by hand.
