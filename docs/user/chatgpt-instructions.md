...
Running workers are not silent black boxes. `codex_worker_status` is the compact pull-based status bar: it exposes active/quiet/stale/lost/completed/failed counts, deltas since the last check, suggested action, one short line per worker, and `recommended_next_poll_seconds`. Default `scope=current` is intentionally not the full historical archive: it shows the current work run plus live/problem workers and reports how many old completed/stopped workers were hidden. Use `scope=conversation` only when deliberately reusing earlier workers from the same ChatGPT conversation, `scope=recent` for recently active workers, and `scope=history` only when the durable archive is needed. Compact status deliberately omits raw shell command text so normal monitoring does not become log reading; use `codex_worker_inspect(view="status")` only when command preview is needed for deliberate debugging. Normal monitoring cadence is about 20-30 seconds between status calls; do not poll every few seconds unless the user explicitly asks for near-real-time monitoring or the status says immediate recovery is needed. If a status call is too soon, PatchBay may return `poll_too_early: true`, `status_current: false`, and `retry_after_seconds`; treat that as "wait, do not panic," because the cached response did not reset activity deltas. Prefer `codex_worker_wait` when you simply need to pause and check again: it waits once, then returns fresh compact status without interrupting workers or exposing raw logs. `codex_worker_inspect(view="compact")` gives the same idea for one worker; `view="status"` exposes the fuller liveness/checkpoint fields. If status shows activity, output growth, or useful partial notes, do not assume the worker is stuck merely because the final report is not ready. Decide whether to wait, continue after completion, or stop deliberately from the compact status first. `codex_worker_message` continues the next turn after completion; active-turn steering is not exposed yet.
