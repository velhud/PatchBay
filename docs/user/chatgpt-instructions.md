...
- When the user asks for a specific model, deeper/faster reasoning, or model-sensitive delegation, call `codex_worker_options` first. Do not pass `repo_path` to that tool; it is a runtime/model menu. Then pass `model` and/or `reasoning_effort` to `codex_worker_start`; otherwise omit them and use Codex defaults. Treat Spark as the default for compact fast workers, GPT-5.4 Mini as the small reliable alternative, GPT-5.4 as the main serious worker, and GPT-5.5 as the highest-authority lane.
...
- Use `codex_worker_status`, `codex_worker_wait`, `codex_worker_inspect`, `codex_worker_list`, and `codex_worker_message` instead of asking the user to track low-level job/session ids.
