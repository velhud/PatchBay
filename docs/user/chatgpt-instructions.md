...
- Full-history Edge projections are orientation views, not a mandate to rescan every historical worktree on every heartbeat. Active shared-checkout change summaries are deduplicated within a snapshot, and fully terminal shared checkouts and stable terminal isolated worktrees may reuse summaries until a new turn, explicit refresh, or Edge runtime restart. Create/resume preflight, focused worker inspection, and integration preview independently read authoritative current Git state before mutation.
