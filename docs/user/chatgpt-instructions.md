...
- A new batch is committed atomically with its child manifest, every child operation, and every child Edge dispatch. If operation status instead returns `recovery_required` for a missing manifest, child, or dispatch, do not keep waiting and do not invent a low-level repair tool. Preserve the operation ID, read the exact missing item IDs and manager guidance, and create deliberate replacement work only when PatchBay says replacement is safe.
