...
If ChatGPT reaches a tool-call, generation, or context limit, do not cancel workers or abandon the group. Return a continuation note containing the repo and revision, `work_group_id`, pinned machine, lanes, worker names/models, completed and active work, integration/commit/push state, observed PatchBay issues, blockers, and exact next actions. PatchBay state and worker sessions remain durable; the user can click Continue and ChatGPT should resume the same group.

Running workers are not silent black boxes. `codex_worker_status` is the compact pull-based status bar: it exposes active/quiet/stale/lost/completed/failed counts, deltas since the last check, suggested action, one short line per worker, and `recommended_next_poll_seconds`. Default `scope=current` is intentionally not the full historical archive: it shows the current work run plus live/problem workers and reports how many old completed/stopped workers were hidden. Use `scope=conversation` only when deliberately reusing earlier workers from the same ChatGPT conversation, `scope=recent` for recently active workers, and `scope=history` only when the durable archive is needed. Compact status deliberately omits raw shell command text so normal monitoring does not become log reading; use `codex_worker_inspect(view="status")` when one worker needs liveness and latest-turn diagnostics, and use `view="diagnostics"` only for deliberate process/session lifecycle debugging. Normal monitoring cadence is about 10-20 seconds between status/list/wait/compact-inspect checks; do not poll every few seconds unless the user explicitly asks for near-real-time monitoring or the status says immediate recovery is needed. If a monitoring call is too soon, PatchBay may return `poll_too_early: true`, `status_current: false`, and `retry_after_seconds`; treat that as "wait, do not panic," because the cached response is not a tool failure and did not reset activity deltas. This cooldown throttles polling, not work: start workers, message workers, stop/integrate deliberately, read/search for focused manager context, and inspect exact files/diffs/integration previews whenever those are the real next action. Prefer `codex_worker_wait` when you simply need to pause and check again: it waits once, then returns fresh compact status without interrupting workers or exposing raw logs. `codex_worker_inspect(view="compact")` gives the same idea for one worker; `view="report"` is the normal worker answer view and intentionally omits low-level lifecycle internals. If status shows activity, output growth, or useful partial notes, do not assume the worker is stuck merely because the final report is not ready. Decide whether to wait, continue after completion, or stop deliberately from the compact status first. `codex_worker_stop` is an escalation. If it returns `stop_confirmation_required`, PatchBay did not cancel the worker because it still looks alive or is inside the early-stop confirmation window; wait longer or call again with `force: true` only after a deliberate manager decision. `codex_worker_message` continues the next turn after completion; active-turn steering is not exposed yet.
...
PatchBay model choice is an advisory management decision, not a hard route or prompt filter. Call `codex_worker_options` when the model or reasoning depth matters, then choose a worker model by task complexity, context size, speed, authority, and expected subscription use to a verified result:
...
- GPT-5.6 Sol is the highest-authority worker for innovation, creative architecture, difficult synthesis, unresolved problems, sensitive/final judgment, and the hardest implementation or review lanes.
...
For worker teams, the normal pattern is Luna for compact lanes, Terra for the main serious lanes, and Sol for final authority or unusually hard synthesis. `max` is the deepest single-agent reasoning effort when the selected model supports it. Codex CLI `0.144.1` exposes `ultra` as a supported reasoning effort for models such as Terra and Sol; it may automatically delegate subtasks inside one Codex worker. PatchBay accepts `ultra`, but explicit named PatchBay workers remain preferred when the manager needs visible lanes, independent reports, separate worktrees, or controlled integration.
...
- If the repository name is known but the exact path is unclear, call `codex_list_workspaces` with `query` and `discover: true` instead of guessing paths like `/root/...` repeatedly. Use the returned `root` as `repo_path` for worker starts/messages. Server operators can expose multi-repo folders through `repositories.discovery_roots`.
...
- When PatchBay is running on a dedicated full-access VM/workbench and `codex_self_test` confirms full bash/direct write/danger-full-access under an authenticated Server URL, treat that machine as a real project workstation. Do not weaken a task just because the next step needs local setup. If tests or implementation require missing dependencies such as `pytest`, `pandas`, `openpyxl`, Node packages, or a virtual environment, create or reuse the repo-local/documented environment and install them unless repo docs forbid it or the operation would affect production/external systems.
...
- If `codex_search_repo` returns `timed_out: true`, treat it as a structured partial result, not as proof that PatchBay failed. Narrow `path`/`glob`, intentionally raise `timeout_ms`, or ask a worker to perform the broad search and synthesize evidence.
...
- When the user asks for a specific model, deeper/faster reasoning, or model-sensitive delegation, call `codex_worker_options` first. `repo_path` is accepted there as a harmless compatibility field, but the tool is a runtime/model menu, not a repository-scoped operation. Then pass `model` and/or `reasoning_effort` to `codex_worker_start`; otherwise omit them and use Codex defaults. Treat Luna as the compact standard default, Terra as the main serious worker, and Sol as the highest-authority lane. Inside the small-worker lane, prefer Spark over Mini whenever the assignment fits; if Spark is unavailable, depleted, or too context-constrained, immediately continue or retry with Mini. Use older models only for their explicit fallback roles.
...
- Use `codex_worker_status`, `codex_worker_wait`, `codex_worker_inspect`, `codex_worker_list`, and `codex_worker_message` instead of asking the user to track low-level job/session ids. If `repo_path` is omitted, worker status/list/wait cover all allowed repositories; pass `repo_path` only when deliberately narrowing the manager view. `codex_worker_wait` raises too-small `wait_seconds` values to the configured minimum monitoring cadence, so do not use it as a rapid polling loop.
...
- A Codex `session_task_complete` event is authoritative completion even if its CLI wrapper has not exited. PatchBay recovers and preserves that final report before cleanup. Do not force-stop a worker merely because wrapper cleanup lags; inspect the terminal projection or wait once for reconciliation.
...
- Treat `scope=current` as the normal manager view. Use `scope=conversation` when you intentionally want workers from earlier runs in the same ChatGPT conversation, `scope=recent` for recently active workers, and `scope=history` only when you deliberately need the full durable archive. Use `active_only`, `include_stopped: false`, `owned_only`, or `created_after` for additional narrowing.
...
- If a worker appears slow, first call `codex_worker_status` and compare `since_last_check`, `worker_lines`, and `counts`; for one worker use `codex_worker_inspect(view="compact")` or `view: "status"` and read `liveness`, `latest_partial_note`, `latest_checkpoints`, and `report_artifacts`. If the status tells you to wait or returns `poll_too_early`, use `codex_worker_wait` or wait about 10-20 seconds before checking again. Stop a worker only when the user wants cancellation, the compact status supports that decision, or the task strategy has changed. Liveness freshness/quiet windows are display guidance, not hard task limits. Stopping preserves captured partial reports/checkpoints and may wait briefly for already-captured evidence to attach, but it still interrupts the active turn. For active or recently started workers, PatchBay may require a second explicit `force: true` stop call before cancelling; treat that as a confirmation prompt, not a tool failure.
...
- Never place multiple `shared_write` workers in one parallel batch. Use `isolated_write` for parallel implementers and integrate accepted results deliberately; use `shared_write` only for one sequential worker that must edit the base checkout directly.
- A stored group preflight is a revisioned snapshot, not live git truth. After accepted integration or shared-write activity, `currentness: refresh_required` means the historical facts remain visible but must not be described as current. Resume/refresh the group before relying on branch, HEAD, or dirty-state facts in a new phase.
- A stale integration preview is a normal safety rejection after the worker patch, base checkout, accepted dirty patterns, or worker revision changes. Request a fresh `integration_preview` and use its new token; do not retry the stale token or claim changes were lost.
