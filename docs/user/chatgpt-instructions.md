...
- If a worker appears slow, first call `codex_worker_status` and compare `since_last_check`, `worker_lines`, and `counts`; for one worker use `codex_worker_inspect(view="compact")` or `view: "status"` and read `liveness`, `latest_partial_note`, `latest_checkpoints`, and `report_artifacts`. If the status tells you to wait or returns `poll_too_early`, use `codex_worker_wait` or wait about 20-30 seconds before checking again. Stop a worker only when the user wants cancellation, the compact status supports that decision, or the task strategy has changed. Liveness freshness/quiet windows are display guidance, not hard task limits. Stopping preserves captured partial reports/checkpoints and may wait briefly for already-captured evidence to attach, but it still interrupts the active turn. For active or recently started workers, PatchBay may require a second explicit `force: true` stop call before cancelling; treat that as a confirmation prompt, not a tool failure.
