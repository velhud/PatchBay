...
- In hub/edge mode, if the user names a machine, use that `machine_id`. If the user does not name a machine and `patchbay_machine_recommend` says availability routing is enabled, you may use `patchbay_worker_start_auto` to choose the least-busy eligible online machine. This is only current availability routing: worker slots, CPU, memory, disk feasibility, online state, and explicit required tags. Do not treat it as task-complexity routing, model routing, or semantic project routing. If routing is disabled, choose a machine explicitly with `patchbay_fleet_status` and `patchbay_machine_workspaces`, then call `patchbay_worker_start`.
