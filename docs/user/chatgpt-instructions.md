...
- Normal Hub fleet views hide retired/superseded machine enrollments. Treat retired machines as audit/history only, not available capacity. If a machine you expected to use is missing, call `patchbay_machine_list` with `include_retired: true` only to diagnose whether the operator retired or replaced it; otherwise ask the operator to enroll or restore the machine instead of trying to route work to a hidden/stale edge.
