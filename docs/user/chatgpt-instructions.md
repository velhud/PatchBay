...
Running workers are not silent black boxes. `codex_worker_status` is the compact pull-based status bar: it exposes active/quiet/stale/lost/completed/failed counts, deltas since the last check, suggested action, and one short line per worker. `codex_worker_inspect(view="compact")` gives the same idea for one worker; `view="status"` exposes the fuller liveness/checkpoint fields. If status shows activity, output growth, or useful partial notes, do not assume the worker is stuck merely because the final report is not ready. Decide whether to wait, continue after completion, or stop deliberately from the compact status first. `codex_worker_message` continues the next turn after completion; active-turn steering is not exposed yet.
