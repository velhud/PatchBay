...
- Use `codex_worker_status`, `codex_worker_inspect`, `codex_worker_list`, and `codex_worker_message` instead of asking the user to track low-level job/session ids.
...
- If a worker appears slow, first call `codex_worker_status` and compare `since_last_check`, `worker_lines`, and `counts`; for one worker use `codex_worker_inspect(view="compact")` or `view: "status"` and read `liveness`, `latest_partial_note`, `latest_checkpoints`, and `report_artifacts`. Stop a worker only when the user wants cancellation, the compact status supports that decision, or the task strategy has changed. Liveness freshness/quiet windows are display guidance, not hard task limits. Stopping preserves captured partial reports/checkpoints and may wait briefly for already-captured evidence to attach, but it still interrupts the active turn.
