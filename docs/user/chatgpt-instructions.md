...
- GPT-5.4 Mini is the quota-saving standard choice for simple, deterministic, high-volume work when maximizing included capacity matters more than Luna's stronger judgment.
...
For worker teams, the normal pattern is Luna for compact lanes, Terra for the main serious lanes, and Sol for final authority or unusually hard synthesis. `max` is a reasoning effort when the selected model supports it. Ultra is a separate multi-agent Codex mode, not a `reasoning_effort` value or PatchBay worker field; use explicit named PatchBay workers when parallel responsibilities are clear.
