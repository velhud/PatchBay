...
- Full-history Edge projections are orientation views, not a mandate to rescan every historical worktree on every heartbeat. Shared-checkout change summaries may be deduplicated within a snapshot, and stable terminal isolated-worktree summaries may be reused until a new turn or Edge runtime restart. Create/resume preflight, focused worker inspection, and integration preview independently read authoritative current Git state before mutation.
