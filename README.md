...
| `codex_worker_list` | List workers with compact `team_status`, liveness lines, checkpoints, and latest report | yes |
| `codex_worker_status` | Show the compact pull-based status bar for active/quiet/stale/lost worker management | yes |
...
Clients can fetch it with `resources/list` and `resources/read`. The MIME type is `text/html;profile=mcp-app`. The current card is derived from the copied CodexPro widget subsystem and renders bounded worker reports, artifact inbox summaries, job status, diffs, direct command/write results, integration previews, ownership/takeover states, and `repo_busy` lock states. It hydrates from both MCP Apps bridge tool-result notifications and ChatGPT `window.openai` compatibility globals, and it shows a compact widget-error state instead of staying on the initial loading skeleton if rendering fails. It remains passive: it does not initiate tool calls. The legacy `ui://widget/patchbay-tool-card-v1.html` URI remains readable for compatibility, while descriptors advertise v2.
