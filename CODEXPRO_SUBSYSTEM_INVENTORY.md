...
The final product remains `codex-mcp-wrapper`. CodexPro code can be copied, ported, or rewritten inside the wrapper as long as MIT attribution is preserved.

## Classification Rules

- `adopt`: copy or port the subsystem as a core part of the wrapper.
...
| CodexPro area | Source | Why it matters | Wrapper target |
| --- | --- | --- | --- |
...
| CodexPro area | Source | Change required | Wrapper target |
| --- | --- | --- | --- |
...
| Profile store | `src/profileStore.ts`, `src/http.ts` | Redact secrets; align with wrapper config; avoid repo-committed profiles | User profile store |
...
| CodexPro area | Source | Reason |
| --- | --- | --- |
| Generic public `read`/`write`/`edit` names | `src/server.ts`, `src/fsOps.ts` | The wrapper should expose Codex-specific intent, not generic file-system verbs. |
...
- do not imply CodexPro upstream owns or endorses the wrapper release.
