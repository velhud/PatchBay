...
def test_initialize_includes_server_instructions():
    ...
    assert "configured worker capacity" in result["instructions"]
    ...
    assert "20-30 seconds between status/list/wait/compact-inspect checks" in result["instructions"]
    ...
    assert "medium as the normal daily-driver effort" in result["instructions"]
    assert "5-10x" in result["instructions"]
    assert "Waiting for healthy workers is part of completing the task" in result["instructions"]
    assert "Never invent an execution or tool-call limit" in result["instructions"]
...
def test_public_tool_call_translates_schema_valid_arguments():
    ...
    assert result["_meta"] == {
        "patchbay/tool_name": "codex_get_status",
        "patchbay/tool_id": "get_status",
    }
...
def test_public_tool_call_passes_request_context_to_handler():
    ...
    assert result["result"]["_meta"]["patchbay/tool_name"] == "codex_get_status"
...
def test_tool_call_returns_redacted_structured_content():
    ...
    assert result["_meta"] == {
        "patchbay/tool_name": "codex_get_config",
        "patchbay/tool_id": "get_config",
    }
...
def test_resume_tool_call_returns_async_job_pointer():
    ...
    assert "tool_name" not in result["structuredContent"]
    assert "tool_id" not in result["structuredContent"]
    assert result["_meta"] == {
        "patchbay/tool_name": "codex_resume",
        "patchbay/tool_id": "resume",
    }
