...
DEFAULT_QUERY_TOKEN_NAMES = ("codex_mcp_token", "codexpro_token", "token")
DEFAULT_TOKEN_ENVS = (
    "CODEX_MCP_HTTP_TOKEN",
    "CODEX_WRAPPER_HTTP_TOKEN",
    "CODEXPRO_HTTP_TOKEN",
...
def build_auth_policy(config: Mapping[str, object], environ: Mapping[str, str] | None = None) -> AuthPolicy:
    ...
    tunnel_mode = str(
        env.get("CODEX_MCP_TUNNEL_MODE")
        or env.get("CODEXPRO_TUNNEL_MODE")
        or auth_config.get("tunnel_mode")
        or "none"
    )
    ...
    if _bool_from(env.get("CODEX_MCP_REQUIRE_HTTP_TOKEN"), False) or _bool_from(
        env.get("CODEXPRO_REQUIRE_HTTP_TOKEN"), False
    ):
    ...
    if reasons and not token:
        raise AuthConfigurationError(
            ...
            "Set CODEX_MCP_HTTP_TOKEN or bind to trusted loopback/local-only mode."
        )
