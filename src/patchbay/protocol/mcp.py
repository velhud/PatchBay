...
3. Model choice should minimize expected subscription use to a verified result, not nominal cost per turn, and remain advisory rather than a deterministic filter. Use GPT-5.6 Luna as the default compact standard worker, GPT-5.6 Terra as the default serious worker for most substantial lanes, and GPT-5.6 Sol for highest-authority, creative, unresolved, sensitive, or final-judgment work. Use Spark only for tiny latency-sensitive work when its separate preview quota is useful; use GPT-5.4 Mini under genuine quota pressure; keep GPT-5.4 and GPT-5.5 as availability, compatibility, or evidence-backed regression fallbacks. Call codex_worker_options because the installed Codex catalog is the authority for availability and supported effort levels. max is a reasoning effort when supported; Ultra is a separate multi-agent mode, not a reasoning_effort value, and PatchBay worker teams should remain explicit named workers.
