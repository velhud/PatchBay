...
3. Model choice should minimize expected subscription use to a verified result, not nominal cost per turn, and remain advisory rather than a deterministic filter. Use GPT-5.6 Luna as the default compact standard worker, GPT-5.6 Terra as the default serious worker for most substantial lanes, and GPT-5.6 Sol for highest-authority, creative, unresolved, sensitive, or final-judgment work. For every bounded small-worker assignment that either Spark or GPT-5.4 Mini could handle, choose Spark first because it is dramatically faster and uses a separate preview quota. If Spark is unavailable, its quota is depleted, or its smaller context is insufficient, immediately continue or retry the same assignment with GPT-5.4 Mini. Keep GPT-5.4 and GPT-5.5 as availability, compatibility, or evidence-backed regression fallbacks. Call codex_worker_options because the installed Codex catalog is the authority for availability and supported effort levels. max is a deep single-agent effort when supported. Codex CLI 0.144.1 exposes ultra for supported models such as Terra and Sol; it may automatically delegate internally inside one worker. Use explicit named PatchBay workers instead when visible lanes, independent reports, separate worktrees, or controlled integration matter.
