...
class PowerToolRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self.repo_locks = repo_locks or RepoMutationLockManager(config)
    ...
    async def run_command(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with self.repo_locks.hold(str(workspace.root), operation="codex_run_command"):
                return await self._run_command_unlocked(command, cwd_path, cwd_rel, args)
        except RepoMutationBusy as busy:
            return busy.public_payload()

    async def _run_command_unlocked(
        self,
        command: str,
        cwd_path: Path,
        cwd_rel: str,
        ...
        if timed_out:
            stderr_text = (stderr_text + f"\n[patchbay] Command timed out after {timeout_ms} ms.").strip()
