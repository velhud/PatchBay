...
- Read-only Edge actions cannot have an uncertain side-effect outcome. A handler error from workspace open/tree/search/read, worker options/inspect, git views, or Pro Request reads must become a sanitized terminal blocked/failed result. Group preflight must preserve explicit failure, and group status must never report `pending` after its preflight operation is terminal.
...
- Keep historical batch uncertainty terminal when it cannot make progress. A pre-atomic batch parent without a durable manifest may be retired as `blocked` only after every observed child is terminal; preserve the observed child evidence, state explicitly that completeness was not proven, never label the parent successful, and never leave it counted as active forever.
