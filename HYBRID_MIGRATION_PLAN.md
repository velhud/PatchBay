...
Transform `codex-mcp-wrapper` into the stronger combined application:

- CodexPro-quality ChatGPT connector and workspace context.
- Wrapper-quality Codex job orchestration, worktree isolation, resume, diff inspection, and auditability.
- Optional power tools without making them the default product identity.

This is not a CodexPro fork. All implementation lands in the wrapper repository.
...
## Phase 1: Stabilize Existing Wrapper Core
...
Status: implemented locally; pending real ChatGPT Developer Mode and public tunnel evals.
...
The launcher now starts from a base `config.yaml`, applies a saved per-workspace profile unless disabled, writes a private runtime config under `CODEX_MCP_HOME` or the user's home directory, prints connector metadata, and starts `server.py` with `CODEX_MCP_CONFIG`. Public URL mode sets tunnel mode and therefore fails closed without `CODEX_MCP_HTTP_TOKEN`.

Optional public tunnel process supervision is now implemented for Cloudflare quick tunnels, Cloudflare named tunnels, and ngrok stable hostnames. Tests use a fake tunnel executable; no real public tunnel is opened during automated verification. The wrapper does not auto-install provider binaries.
...
- Keep neutral/CodexPro-compatible aliases controlled by `app.tool_mode`.
...
- Do not turn the wrapper into a CodexPro fork.
